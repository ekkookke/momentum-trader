...
class MarketDataLoader:
    ...
    def load_etf(self, etf: ETFConfig) -> pd.DataFrame:
        ...
        key = self.cache.key("etf", etf.symbol, start, end, self.config.data.adjust)
    ...
    def load_benchmark(self) -> pd.DataFrame:
        ...
        key = self.cache.key("index", symbol, start, end, "none")
        df = self.cache.read_or_fetch(
            key,
            lambda: self.client.fetch_index_daily(symbol=symbol, start_date=start, end_date=end),
        )
        return self._clip_valid_history(df, start, end)
